...
- **All pop-up panels center on screen**, per the confirmed UI edit — this was an explicit fix from the storyboard-review round, don't regress it. Confirmation modals follow Doctrine §23 (lightweight for reversible actions, a stronger centred modal for deletion/reassignment/submission/high-impact AI content). **One confirmed exception (Stage 9):** the Notifications panel is a right-edge floating drawer, not centered — a deliberate Product Owner decision, not a regression. This is the only carve-out to this rule; every other pop-up/dialog/modal still centers.
